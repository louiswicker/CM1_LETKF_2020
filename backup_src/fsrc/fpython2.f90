...
! These parameters are for the Relaxation to Prior Spread (RTPS)
  ...
  real(kind=4), parameter :: inflate_RTPS = 0.9, eps = 1.0e-10

! These parameters bind the maximum amount of multiplicative inflation used (LETKF AI and RTPS)

  real(kind=4), parameter :: min_inflate = 1.00, max_inflate = 2.0
...
! Logic for inflation, set defaults, and initialize out_inflate since not all grid points are processed.
    ...
    IF( inflate_flag .eq. 4 ) THEN
      write(std_out,*) "COMPUTE_LETKF, ADAPTIVE RELAXATION TO PRIOR PERTURBATION (RTPP) is ON"
      write(std_out,*) "Minimum coefficient for increments:  ", inflate_RTPP/2.0
      write(std_out,*) 
    ENDIF
...
!======================================================================================================'
!
! Routine to map raw dbz obs to model grid for additive noise calcs.  Nearest neighbor algorithm
!
! 
!
!======================================================================================================'
SUBROUTINE GRID_DBZ_OBS(dbz, obs, xob, yob, zob, xc, yc, zc, hscale, vscale, nobs, nx, ny, nz)
                   
  implicit none

! Passed in variables

  real(kind=4), INTENT(OUT) :: dbz(nz,ny,nx)    ! 3D bubbles passed back to calling routine
      
  integer,      INTENT(IN)  :: nx, ny, nz, nobs  ! grid dimensions
  real(kind=8), INTENT(IN)  :: xob(nobs)         ! coords. for nb bubbles relative to SW corner of domain
  real(kind=8), INTENT(IN)  :: yob(nobs)         ! coords. for nb bubbles relative to SW corner of domain
  real(kind=8), INTENT(IN)  :: zob(nobs)         ! coords. for nb bubbles relative to SW corner of domain      
  real(kind=8), INTENT(IN)  :: obs(nobs)         ! coords. for nb bubbles relative to SW corner of domain      
  real(kind=8), INTENT(IN)  :: xc(nx)            ! coordinates corresponding to grid indices
  real(kind=8), INTENT(IN)  :: yc(ny)            ! coordinates corresponding to grid indices
  real(kind=8), INTENT(IN)  :: zc(nz)            ! coordinates corresponding to grid indices
  real(kind=8), INTENT(IN)  :: hscale, vscale

! Local variables

  integer i, j, k, iob, job, kob, o, obcount, n      ! loop variables
  integer, external :: find_index8
  
  logical, parameter :: debug = .false.
  
  logical, parameter :: nearest = .false.
! real, parameter :: hscale = 5000.0, vscale = 1000.
  real(kind=8) top, wgt, dh, dv, wgt_sum

  dbz(:,:,:) = 0.0
  
  IF( debug) THEN
    print *, "FORTRAN GRID_DBZ_OBS:  dims  ", nx, ny, nz, nobs
    print *, "FORTRAN GRID_DBZ_OBS:    obs ", minval(obs), maxval(obs)
    print *, "FORTRAN GRID_DBZ_OBS:  x-obs ", minval(xob), maxval(xob)
    print *, "FORTRAN GRID_DBZ_OBS:  y-obs ", minval(yob), maxval(yob)
    print *, "FORTRAN GRID_DBZ_OBS:  z-obs ", minval(zob), maxval(zob)
    print *, "FORTRAN GRID_DBZ_OBS:  x-grid", minval(xc), maxval(xc)
    print *, "FORTRAN GRID_DBZ_OBS:  y-grid", minval(yc), maxval(yc)
    print *, "FORTRAN GRID_DBZ_OBS:  z-grid", minval(zc), maxval(zc)
  ENDIF
  ...
  IF( nearest ) THEN
				...
				DO o = 1,nobs
						...
						iob = find_index8(xob(o), xc, nx)
						job = find_index8(yob(o), yc, ny)
						kob = find_index8(zob(o), zc, nz)

						IF (iob .gt. 0  .and. iob .le. nx .and. &
										job .gt. 0  .and. job .le. ny .and. &
										kob .gt. 0  .and. kob .le. nz ) THEN      
									
							obcount = obcount + 1

							dbz(kob, job, iob) = max(dbz(kob, job, iob), obs(o))
							IF ( iob+1 .le. nx ) dbz(kob, job, iob+1) = max(dbz(kob, job, iob+1), 0.5*(obs(o)+dbz(kob, job, iob+1)) )
							IF ( job+1 .le. ny ) dbz(kob, job+1, iob) = max(dbz(kob, job+1, iob), 0.5*(obs(o)+dbz(kob, job+1, iob)) )
							IF ( kob+1 .le. nz ) dbz(kob+1, job, iob) = max(dbz(kob+1, job, iob), 0.5*(obs(o)+dbz(kob+1, job, iob)) )

							IF ( iob-1 .gt. 0 ) dbz(kob, job, iob-1) = max(dbz(kob, job, iob-1), 0.5*(obs(o)+dbz(kob, job, iob-1)) )
							IF ( job-1 .gt. 0 ) dbz(kob, job-1, iob) = max(dbz(kob, job-1, iob), 0.5*(obs(o)+dbz(kob, job-1, iob)) )
							IF ( kob-1 .gt. 0 ) dbz(kob-1, job, iob) = max(dbz(kob-1, job, iob), 0.5*(obs(o)+dbz(kob-1, job, iob)) )

						ENDIF
				
				ENDDO     ! END OBS LOOP
				  
				print *, "GRID_DBZ_OBS:  ",obcount," observations gridded"
         ...
         DO i = 1,nx
           ...
           DO n = 1,nobs
             dh = ( (xc(i)-xob(n))**2 + (yc(j)-yob(n))**2) / hscale**2
												 dv = ( (zc(k)-zob(n))**2) / vscale**2                                 
				        ...
				        IF (wgt_sum .ge. 0.01) THEN
              dbz(k,j,i) = dbz(k,j,i) + top/wgt_sum
            ENDIF
				    ENDDO				
				  ENDDO				
				ENDDO
...
SUBROUTINE GRID3D_DBZ_OBS(dbz, obs, xob, yob, zob, xc, yc, zc, hscale, vscale, nobs, nx, ny, nz)
                   
  implicit none

! Passed in variables

  real(kind=4), INTENT(OUT) :: dbz(nz,ny,nx)    ! 3D analysis passed back to calling routine
  ...
  real(kind=8), INTENT(IN)  :: zob(nobs)         ! coords. for nb bubbles relative to SW corner of domain      
  real(kind=8), INTENT(IN)  :: obs(nobs)         ! coords. for nb bubbles relative to SW corner of domain      
  real(kind=8), INTENT(IN)  :: xc(ny,nx)         ! coordinates corresponding to grid indices
  real(kind=8), INTENT(IN)  :: yc(ny,nx)         ! coordinates corresponding to grid indices
  real(kind=8), INTENT(IN)  :: zc(nz,ny,nx)      ! coordinates corresponding to grid indices
  real(kind=8), INTENT(IN)  :: hscale, vscale

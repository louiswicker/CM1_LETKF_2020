...
if options.exp == None:
    parser.print_help()
    print("\n ==> ENS_MAIN: ERROR --> Experiment file not supplied..EXITING!!!")
    sys.exit(-1)
else:
...
if options.datetime == None:
    ...
    print("\n ==> ENS_MAIN: Date and time supplied is %s" % (myDT.strftime("%Y %m-%d %H:%M:%S")))
...
if options.member == None:
    print
    print("\nNo member supplied, using default member: %d\n" % _member)
    print
    member = _member
else:
...
for f in exper['fcst_members']:
    files.append(os.path.join(f, ("cm1out_rst_%6.6d.nc" % number)))
...
plot_W_DBZ_T_WZ(wplot, dplot, tplot, wzplot, xc, yc, height, myDT, \
